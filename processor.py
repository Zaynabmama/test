...
from budg.quarter_utils import (
    QUARTER_ORDER,
...
def customer_summary(df, selected_quarter="Q1"):
    ...
    # Existing pivots for each quarter
    for quarter, (start_dt, end_dt) in quarter_bounds.items():
        out[f"{quarter}-2026 - pivot"] = np.where(
            (due_dt >= start_dt) & (due_dt <= end_dt), ar_val, 0
        )
    ...
    final_order = [
        ...
        cfg["current_pivot_label"], cfg["current_period_label"], cfg["percent_label"], cfg["actual_label"],
        *cfg["tail_labels"],
        cfg["remaining_label"], cfg["to_add_label"], cfg["forecast_label"],
        *cfg["later_quarter_labels"], *cfg["year_labels"]
    ]
